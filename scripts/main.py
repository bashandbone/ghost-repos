...
CODE_LANGUAGES = {
    ...
    "c#", "ruby", "kotlin", "swift", "scala", "elixir", "haskell", "lua",
    "zig", "nim", "ocaml", "clojure", "erlang", "dart", "r", "julia",
    "perl", "php", "objective-c", "assembly", "fortran", "ada", "d",
    "crystal", "v", "odin", "gleam", "racket", "scheme", "common lisp",
    "f#", "visual basic .net", "powershell",
}
    ...
    with open(path) as f:
        ...
        for row in reader:
            repos.append(GhostRepo(
                ...
                stars_gained_2025=int(row.get("stars_2025", row.get("stars_gained_2025", 0))),
...
def stage2_inspect(client: httpx.Client, repo: GhostRepo) -> None:
    ...
    langs = api_get(client, f"https://api.github.com/repos/{repo.repo_name}/languages")
    if langs:
    ...
    if repo.languages_breakdown:
        has_code = any(
            lang.lower() in CODE_LANGUAGES
            for lang in repo.languages_breakdown
        )
        if not has_code:
    ...
    contents = api_get(
        client,
        f"https://api.github.com/repos/{repo.repo_name}/contents"
    )
    if contents and isinstance(contents, list):
        names = [item.get("name", "").lower() for item in contents]
        repo.has_readme = any(n.startswith("readme") for n in names)
        repo.has_tests = any(
            n in ("tests", "test", "__tests__", "spec", "specs", "test.py",
                   "tests.py", "pytest.ini", "jest.config.js", "vitest.config.ts")
            for n in names
        )
        repo.has_ci = any(
            n in (".github", ".gitlab-ci.yml", ".circleci", ".travis.yml",
                   "jenkinsfile", ".drone.yml")
            for n in names
        )
        repo.has_docs = any(
            n in ("docs", "doc", "documentation", "wiki", "guide")
            for n in names
        )
    ...
    releases = api_get(
        client,
        f"https://api.github.com/repos/{repo.repo_name}/releases?per_page=5"
    )
    ...
    commits = api_get(
        client,
        f"https://api.github.com/repos/{repo.repo_name}/commits?per_page=30"
    )
...
def print_results(repos: list[GhostRepo], top_n: int = 50) -> None:
    ...
    for i, repo in enumerate(repos[:top_n], 1):
        signals = []
        if repo.has_readme: signals.append("📄")
        if repo.has_tests: signals.append("🧪")
        if repo.has_ci: signals.append("⚙️")
        if repo.has_docs: signals.append("📚")
        if repo.release_count_api > 0: signals.append(f"🏷️×{repo.release_count_api}")

        age_str = f"{repo.repo_age_days // 365}y" if repo.repo_age_days >= 365 else f"{repo.repo_age_days // 30}mo"
        ...
        table.add_row(
            ...
            " ".join(signals),
            (repo.description[:37] + "...") if len(repo.description) > 37 else repo.description,
        )
    console.print(table)
...
def save_results(repos: list[GhostRepo], path: Path) -> None:
    fields = [
        "repo_name", "total_stars", "repo_age_days", "push_events",
        "total_commits", "active_months", "active_weeks",
        "pr_events", "pr_reviews", "issues_opened", "issue_comments",
        "project_activity_score", "releases", "tags_created",
        "human_pushers", "language", "license", "description",
        "created_at", "is_fork",
        "has_readme", "readme_length", "has_tests", "has_ci", "has_docs",
        "release_count_api", "commit_message_diversity",
        "quality_score", "ghost_score", "combined_score",
        "html_url", "topics", "languages_breakdown",
    ]
...
def main() -> None:
    ...
    parser.add_argument("--output", "-o", type=Path, default=Path("ghost_repos_v5_final.csv"))
    ...
    if token:
        ...
        console.print("[red]No token — this will be very slow. Use --token or set GITHUB_TOKEN.[/red]")
        sys.exit(1)

    # Rate limit check
    with httpx.Client(headers=headers, timeout=30, follow_redirects=True) as client:
        rl = api_get(client, "https://api.github.com/rate_limit")
        if rl:
    ...
    with httpx.Client(headers=headers, timeout=30, follow_redirects=True) as client:
        for repo in track(repos, description="Stage 1..."):
            if not stage1_verify(client, repo):
                reason = "fork" if repo.is_fork else "archived" if repo.is_archived else "filtered"
            ...
            if repo.language and repo.language.lower() not in CODE_LANGUAGES:
                stage1_skipped[f"lang:{repo.language}"] = stage1_skipped.get(f"lang:{repo.language}", 0) + 1
                continue

            stage1_passed.append(repo)

    console.print(f"Stage 1: {len(stage1_passed)} passed, {sum(stage1_skipped.values())} skipped")
    ...
    console.print(f"[dim]~5 API calls per repo, ~{len(stage1_passed) * 5 * 1.5 / 60:.0f} minutes[/dim]")
    ...
    if final:
        ...
        console.print(f"That's {total_commits / max(total_stars, 1):,.0f} commits per star.[/dim]")
